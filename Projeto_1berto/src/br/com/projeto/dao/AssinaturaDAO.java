...
public class AssinaturaDAO {
	...
	public Assinatura getAssinatura(int codigo)throws Exception{
		stmt = con.prepareStatement
				("SELECT * FROM RW_T_ASSINATURA WHERE ID_ASSINATURA=?");
	...
	public Assinatura getAssinatura(int codigo)throws Exception{
		stmt = con.prepareStatement
				("SELECT * FROM RW_T_ASSINATURA INNER JOIN "
+ "RW_T_USUARIO ON "
+ "RW_T_ASSINATURA.CD_USUARIO = RW_T_USUARIO.CD_USUARIO "
+ "WHERE ID_ASSINATURA=?");
		stmt.setInt(1, codigo);
		rs = stmt.executeQuery();
		if (rs.next()) {
	...
	public int addAssinatura(Assinatura a) throws Exception{
		stmt = con.prepareStatement
			("INSERT INTO RW_T_ASSINATURA "
		+ "(CD_ASSINATURA, TP_ASSINATURA, VL_ASSINATURA, DT_ASSINATURA, ID_USUARIO) "
		+ "VALUES (?,?,?,?,?)");
	...
	public boolean verificarUsuario(int codigoUsuario) throws Exception{
		stmt = con.prepareStatement
		("SELECT * FROM RW_T_ASSINATURA WHERE CD_USUARIO=?");
