...
public class UsuarioBO {
	...
	public int novoUsuario(Usuario objUsuario) throws Exception {
		...
		if (objUsuario.getSenha().length()<3 ||
				objUsuario.getSenha().length()>15 ) {
			throw new RuntimeException("Senha deve estar entre 3 e 15");
		}
		objUsuario.setNome(objUsuario.getNome().toUpperCase());
		UsuarioDAO dao = new UsuarioDAO();
		//if (dao.getUser(objUsuario.getCodigo()).getCodigo()==0) {
		Usuario resultado = dao.getUser(objUsuario.getCodigo());
